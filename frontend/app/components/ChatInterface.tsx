...
export default function ChatInterface({ isDarkMode }: ChatInterfaceProps) {
  ...
  const handlePayNow = async () => {
    ...
    try {
      console.log('💰 Starting PYUSD transaction');
      ...
      console.log('🎯 Getting AI response with payment proof');
  ...
  const handleManualSendPYUSD = async () => {
    try {
      console.log('💰 Starting manual PYUSD send process');
      ...
      // Convert the human-readable amount to raw units (6 decimals for PYUSD)
      const value = BigInt(Math.floor(parseFloat(amountInput) * Math.pow(10, 6)));
      ...
      console.error('💥 Error sending PYUSD:', err);
      alert(`Failed to send PYUSD: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  };
        ...
        <div className="flex items-center justify-between max-w-4xl mx-auto w-full">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="px-3 py-2 text-sm text-[#004f4f]/60 hover:text-[#004f4f] hover:bg-[#004f4f]/5 rounded-lg transition-colors flex items-center gap-2"
